...
Tu peux installer l'app depuis le Shopify App Store **avant** de créer ton compte Actero. Dans ce cas :

1. Actero crée automatiquement ton espace de travail à partir du nom et de l'email de ta boutique Shopify.
2. À la fin de l'installation, la page de succès te propose **Créer mon compte** (ou **J'ai déjà un compte** si tu en as un).
3. Dès que tu es connecté — juste après l'inscription ou à ta première ouverture du dashboard — ta boutique est rattachée automatiquement à ton compte. Rien à reconnecter.

Tu as **30 jours** pour créer ton compte après l'installation. Passé ce délai, relance simplement le flow depuis **Apps → Actero** dans Shopify Admin. Une boutique déjà rattachée à un compte ne peut pas être récupérée par un autre compte.
