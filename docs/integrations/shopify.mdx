...
<Steps>
  ...
  <Step title="Approuve l'installation">
    Tu reviens automatiquement sur `actero.fr`, sur une page qui confirme que ta boutique est connectée. Aucune carte bancaire requise côté Shopify — Actero est facturée séparément depuis ton compte Actero.
  </Step>
  <Step title="Crée ton compte Actero (première installation seulement)">
    Si tu découvres Actero par l'App Store, tu n'as pas encore de compte : la page de confirmation te propose d'en créer un. **Ta boutique se rattache automatiquement au compte dès l'inscription** — tu n'as aucun identifiant de boutique à recopier, aucun jeton à coller.

    Si tu avais déjà un compte Actero, cette étape est sautée : la boutique est rattachée immédiatement et tu arrives directement sur ton dashboard.
  </Step>
...
### Installer sans compte Actero

Tu peux installer l'app depuis le Shopify App Store **avant** de créer ton compte Actero : Actero crée automatiquement ton espace de travail à partir du nom et de l'email de ta boutique Shopify, et la page de confirmation te propose de créer ton compte (voir l'étape 4 ci-dessus). Le rattachement reste valable **30 jours** — voir la [FAQ](#faq). Une boutique déjà rattachée à un compte ne peut pas être récupérée par un autre compte.
...
- **Boutique connectée mais pas encore rattachée à un compte** → page de confirmation avec la création de compte
...
<AccordionGroup>
  ...
  <Accordion title="J'ai installé l'app mais je n'ai pas créé mon compte tout de suite. C'est perdu ?">
    Non. Le rattachement de ta boutique reste valable **30 jours** après l'installation. Reviens sur la page de confirmation depuis **Shopify Admin → Apps → Actero**, crée ton compte, et la boutique se rattache. Passé ce délai, il suffit de relancer l'installation depuis Shopify Admin.
  </Accordion>
  <Accordion title="Combien de temps pour indexer toute ma boutique ?">
    Les 30 dernières commandes sont indexées en ~20 secondes (immédiat). L'indexation complète (catalogue, clients, 60 derniers jours de commandes) prend de 2 à 30 minutes selon la taille de ta boutique. Tu reçois une notification dans le dashboard quand c'est fini. L'historique de commandes est limité à 60 jours : Shopify réserve les commandes plus anciennes aux apps disposant du scope `read_all_orders`, qu'Actero ne demande volontairement pas.
  </Accordion>
