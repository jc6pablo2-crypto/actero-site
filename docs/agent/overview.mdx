...
Par défaut, ton agent utilise **Claude Sonnet 5** d'Anthropic, servi via **OpenRouter**, pour la classification et la génération. Le coût moyen mesuré est d'environ **0,011 $ par ticket**, variable selon la taille de ta base de connaissances. Si OpenRouter est indisponible, l'agent bascule automatiquement sur l'API Anthropic directe : la panne d'un fournisseur ne bloque pas tes réponses.
...
<AccordionGroup>
  ...
  <Accordion title="Que se passe-t-il si le fournisseur IA est down ?">
    L'agent bascule automatiquement sur un fournisseur de secours : si OpenRouter échoue, l'appel repart vers l'API Anthropic directe. Si les deux échouent, le run est marqué `error` et le ticket est automatiquement escaladé vers **À traiter** avec la raison `error`. Aucun client ne reçoit de réponse cassée.
  </Accordion>
</AccordionGroup>
