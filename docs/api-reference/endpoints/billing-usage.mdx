...
Pour suivre la consommation en temps réel **sans** poller cet endpoint en continu, abonne-toi aux événements `usage.threshold_reached` (80 % du quota) et `usage.quota_reached` (100 % — agent en pause) via un [webhook sortant](/api-reference/webhooks). Chaque événement est émis une seule fois par mois. Utile pour :
