...
En aval, les actions irréversibles (réponse au customer, email, escalade) sont réservées par événement avant d'être exécutées : si un même événement est retraité — retry de cron, webhook redélivré, exécutions concurrentes — l'action ne part qu'une seule fois. Seule une tentative en échec (rien n'est parti) est rejouée.
