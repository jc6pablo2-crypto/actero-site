...
| ------------------------------------ | ----------------- | ------- |
| `/api/auth/signup`                   | 5 / IP            | 60 min  |
| `/api/auth/send-verification-code`   | 5 / IP            | 60 min  |
| `/api/auth/verify-code`              | 15 / IP           | 60 min  |
| `/api/copilot-chat`                  | 30 / user         | 60 s    |
| Widget (`/api/engine/webhooks/widget`) | 20 / clé API + IP | 60 s    |

Ces compteurs sont **partagés entre toutes les instances serverless** : la limite annoncée est la limite appliquée, quel que soit le nombre d'instances que la plateforme ouvre. L'incrément est atomique, donc deux requêtes simultanées ne peuvent pas passer toutes les deux sur le même dernier jeton.

Si le compteur partagé est temporairement injoignable, Actero retombe sur un compteur local par instance plutôt que de refuser tout le trafic. L'API reste disponible pendant l'incident. La limite effective peut alors être légèrement plus permissive, jamais plus stricte.
...
<Note>
Le header `Retry-After` est exposé sur les routes dashboard et widget (copilot, checkout, widget). Sur l'engine gateway et les routes auth, il n'est pas envoyé systématiquement — calcule ton retry sur la fenêtre de la limite (10 min, 60 min selon le cas).
</Note>
...
Pour des volumes Enterprise (>50 000 tickets/mois ou >1 000 messages/heure), demande au support un plafond dédié. Le rate limit est déjà appliqué de façon cluster-wide : inutile de dimensionner tes retries en fonction du nombre d'instances.
