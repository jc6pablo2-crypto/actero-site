---
title: "Rate limits et quotas mensuels de l'API Actero"
description: "Limites de l'API Actero par client, customer et IP, quotas mensuels de tickets avec hard cap, réponses 429 et stratégie de retry recommandée."
---
...
| Plan       | Tickets / mois | Au-delà du quota                       |
| ---------- | -------------- | -------------------------------------- |
...
Tous les plans sont en **hard cap** : au-delà du quota, la requête est refusée avec `429` (le widget de chat renvoie un message neutre au client final). Les crédits ponctuels sont la seule façon de continuer à servir sans upgrader — chaque ticket au-delà du quota décrémente 1 crédit.
...
Les routes engine n'exposent pas de headers `X-RateLimit-Remaining` ni `X-RateLimit-Reset` aujourd'hui. Pour estimer ta consommation en temps réel, utilise [`GET /api/billing/usage`](/api-reference/endpoints/billing-usage) qui retourne `tickets_used` / `tickets_limit` / `credits_balance` du mois en cours.

## Augmenter tes limites

| Besoin                                  | Action                                                    |
| --------------------------------------- | --------------------------------------------------------- |
| Quota mensuel saturé                    | Achète un pack de crédits ou upgrade vers un plan supérieur |
