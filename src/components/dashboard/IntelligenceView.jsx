    ...
    <>
      ...
      <div className="fixed inset-y-0 right-0 z-50 w-full md:w-[540px] bg-[#F9F7F1] shadow-2xl flex flex-col transform transition-transform duration-300 translate-x-0 overflow-y-auto border-l border-gray-200">
        <div className="p-6 md:p-8 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-[#F9F7F1]/90 backdrop-blur z-10">
          ...
          <button
            onClick={onClose}
            className="p-2 bg-gray-50 border border-gray-200 rounded-full text-[#716D5C] hover:text-[#262626] hover:bg-gray-50 transition-colors shadow-sm"
          >
            <X className="w-5 h-5" />
          </button>
        </div>

        <div className="p-6 md:p-8 flex-1 flex flex-col gap-8 animate-fade-in-up font-light text-[#716D5C]">
          <section>
            <span className="text-xs font-bold px-3 py-1 rounded-full bg-gray-50 text-[#716D5C] border border-gray-200 mb-4 inline-block uppercase tracking-widest">
              Contexte de l'action
            </span>
            <h3 className="text-2xl font-bold text-[#262626] mb-2 leading-tight tracking-tight">
              {reco.title}
            </h3>
            <p className="text-sm font-normal text-[#716D5C] leading-relaxed mb-6">
              {reco.description}
            </p>

            <div className="flex gap-4 p-5 rounded-2xl bg-white border border-gray-100">
              <div className="flex-1">
                <p className="text-[10px] font-bold text-[#716D5C] uppercase tracking-widest mb-1.5">
                  Score d'impact IA
                </p>
                <span className="text-2xl font-black text-[#262626]">
                  {reco.impact_score}
                  <span className="text-sm font-bold text-[#716D5C]">/100</span>
                </span>
              </div>
              ...
              <div className="flex-1">
                ...
                <span className="text-sm font-bold text-[#262626] capitalize">
                  {reco.category}
                </span>
              </div>
            </div>
          ...
          {(reco.estimated_time_gain_minutes > 0 || reco.estimated_revenue_gain > 0) && (
            <section>
              <h4 className="text-xs font-bold text-[#716D5C] mb-4 uppercase tracking-widest">
          ...
          <section className="relative">
            <h4 className="text-xs font-bold text-[#716D5C] mb-6 uppercase tracking-widest">
              ...
              {planSteps.map((step, idx) => (
                <div key={idx} className="relative flex items-start gap-4">
                  ...
                  <div className="bg-[#F9F7F1] border border-gray-100 p-4 rounded-2xl flex-1 shadow-sm mt-1">
                    ...
                    <p className="text-sm font-normal text-[#716D5C] leading-relaxed">
                      {step.desc}
                    </p>
                  </div>
                </div>
          ...
          {isStarted && (
            <section className="mt-4 bg-white border border-gray-200 rounded-3xl p-6 text-[#262626] overflow-hidden relative">
              ...
              <p className="text-sm text-[#716D5C] font-normal mb-6">
                Les agents parcourent l'infrastructure actuellement.
              </p>

              {recentEvents.length === 0 ? (
                <div className="py-8 text-center flex flex-col items-center">
                  <RefreshCw className="w-6 h-6 text-[#716D5C] animate-spin mb-3" />
                  <p className="text-sm font-bold text-[#716D5C]">
                ...
                <div className="space-y-3 mb-6">
                  {recentEvents.map((evt, i) => (
                    <div
                      key={i}
                      className="bg-gray-50 border border-gray-200 p-3 rounded-xl flex items-center justify-between text-sm animate-fade-in-up"
                    >
                      ...
                      <span className="text-xs text-[#716D5C] font-medium">
                        À l'instant
                      </span>
                    </div>
                  ))}
        ...
        {!isStarted && (
          <div className="p-6 md:p-8 border-t border-gray-200 bg-white flex flex-col sm:flex-row gap-3 sticky bottom-0 z-10">
            <button
              disabled={loading}
    ...
    <div className={`border rounded-3xl p-6 transition-all duration-300 ${isLight ? "bg-white border-gray-200 shadow-sm hover:shadow-md" : "bg-[#F9F7F1] border-gray-200 hover:border-gray-300"}`}>
      <div className="flex flex-col lg:flex-row gap-6 mb-6">
        <div className="flex-1">
          ...
          <h4 className={`text-xl font-bold mb-2 tracking-tight ${isLight ? "text-[#262626]" : "text-[#262626]"}`}>
            {reco.title}
          </h4>
          <p className={`text-sm font-medium leading-relaxed mb-4 ${isLight ? "text-[#716D5C]" : "text-[#716D5C]"}`}>
            {reco.description}
          </p>

          <div className="flex flex-wrap gap-6 mt-4">
            <div className="flex flex-col gap-1">
              <span className={`text-[10px] font-bold uppercase tracking-widest ${isLight ? "text-[#716D5C]" : "text-[#716D5C]"}`}>
                Impact estimé
              </span>
              <div className="flex items-center gap-3">
                <span className={`text-lg font-bold ${isLight ? "text-[#262626]" : "text-[#262626]"}`}>
                  {reco.impact_score}/100
                </span>
                <div className={`h-1.5 w-24 rounded-full overflow-hidden ${isLight ? "bg-gray-100" : "bg-gray-50"}`}>
                  <div
      ...
      <div className={`pt-6 border-t flex flex-wrap items-center gap-3 ${isLight ? "border-gray-100" : "border-gray-100"}`}>
        <button
    ...
    <div className="flex flex-col gap-6">
      <div className={`border rounded-3xl shadow-sm overflow-hidden transition-colors duration-300 ${isLight ? "bg-white border-gray-200" : "bg-[#F9F7F1] border-gray-200"}`}>
        <div className={`p-6 md:p-8 border-b flex flex-col md:flex-row justify-between items-start md:items-center gap-4 ${isLight ? "bg-white text-[#262626]" : "bg-white text-[#262626]"}`}>
          <div>
            <div className="flex items-center gap-3 mb-2">
              <div className="p-2 bg-gray-50 rounded-xl backdrop-blur-md border border-gray-200 shadow-sm">
          ...
          <button
            onClick={() => refetch()}
            disabled={isLoading}
            className={`text-sm font-bold px-4 py-2 rounded-xl shadow-sm transition-all flex items-center gap-2 disabled:opacity-50 ${isLight ? "bg-white text-[#262626] hover:bg-gray-100" : "bg-gray-50 text-[#262626] hover:bg-gray-50"}`}
          >
        ...
        <div className={`px-6 py-4 flex flex-wrap gap-4 items-center border-b ${isLight ? "bg-[#F9F7F1] border-gray-100" : "bg-[#F9F7F1] border-gray-100"}`}>
          ...
          <select
            ...
            className={`border text-sm font-bold rounded-lg px-3 py-1.5 focus:ring-2 outline-none transition-colors ${isLight ? "bg-white border-gray-200 text-slate-700" : "bg-gray-50 border-gray-200 text-[#716D5C]"}`}
          >
          ...
          <select
            ...
            className={`border text-sm font-bold rounded-lg px-3 py-1.5 focus:ring-2 outline-none transition-colors ml-auto ${isLight ? "bg-white border-gray-200 text-slate-700" : "bg-gray-50 border-gray-200 text-[#716D5C]"}`}
          >
      ...
      {error ? (
          ...
          {[...Array(3)].map((_, i) => (
            <div key={i} className={`border rounded-3xl p-6 shadow-sm animate-pulse h-48 flex flex-col gap-4 ${isLight ? "bg-white border-gray-200" : "bg-[#F9F7F1] border-gray-200"}`}>
              ...
              <div className="mt-auto flex gap-4">
                <div className={`h-10 rounded w-32 ${isLight ? "bg-gray-100" : "bg-gray-50"}`}></div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className={`border rounded-3xl p-16 text-center shadow-sm flex flex-col items-center ${isLight ? "bg-white border-gray-200" : "bg-[#F9F7F1] border-gray-200"}`}>
          ...
          <p className={`font-bold text-xl mb-2 ${isLight ? "text-[#262626]" : "text-[#262626]"}`}>Tout est optimisé</p>
          <p className={`font-medium max-w-md ${isLight ? "text-[#716D5C]" : "text-[#716D5C]"}`}>L'IA n'a pas de nouvelle recommandation à proposer pour le moment.</p>
        </div>
      ) : (
