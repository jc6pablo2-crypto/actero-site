  ...
  return (
    <div data-tour="sidebar" className="w-full md:w-[230px] flex flex-col h-full bg-white">
      ...
      <div className="px-3 pt-1 pb-2">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 py-2 rounded-xl bg-[#fafafa] border border-[#f0f0f0] text-[12px] text-[#1a1a1a] placeholder:text-[#9ca3af] focus:outline-none focus:border-[#0F5F35]/30 focus:bg-white transition-all"
          />
        </div>
      </div>
        ...
        {items.map((item, idx) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full h-8 flex items-center justify-between pl-9 pr-2.5 rounded-lg text-[12px] transition-all duration-150 ${
                            isActive
                              ? "text-[#0F5F35] font-semibold bg-[#0F5F35]/[0.08] border-l-2 border-[#0F5F35]"
                              : "text-[#71717a] font-normal hover:text-[#1a1a1a] hover:bg-[#fafafa]"
                          }`}
                        >
                          <div className="flex items-center gap-2">
                            {child.icon && <child.icon className={`w-3.5 h-3.5 ${isActive ? 'text-[#0F5F35]' : 'text-[#9ca3af]'}`} />}
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2.5">
                {item.icon && (
                  <item.icon className={`w-4 h-4 ${isActive ? 'text-[#0F5F35]' : 'text-[#9ca3af] group-hover:text-[#71717a]'}`} />
                )}
                <span>{item.label}</span>
              </div>
              {item.badge && (
      ...
      <div className="relative border-t border-[#f0f0f0]" ref={accountRef}>
        ...
        >
          <div className="w-8 h-8 rounded-lg bg-[#0F5F35] text-white flex items-center justify-center text-[11px] font-bold flex-shrink-0">
            {initials}
          </div>
