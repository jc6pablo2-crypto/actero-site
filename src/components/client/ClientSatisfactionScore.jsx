  ...
  return (
    <div className={`rounded-xl border p-4 ${isLight ? 'bg-white border-gray-200' : 'bg-white border-gray-100 shadow-sm'}`}>
      <div className="flex items-center gap-2 mb-2">
        ...
        <p className={`text-xs font-bold uppercase tracking-wider ${isLight ? 'text-[#71717a]' : 'text-[#71717a]'}`}>{label}</p>
      </div>
      <p className={`text-2xl font-bold ${isLight ? 'text-[#1a1a1a]' : 'text-[#1a1a1a]'}`}>
        {value}<span className="text-sm font-normal text-[#71717a] ml-1">{unit}</span>
      </p>
    </div>
  )
}
    ...
    <div className="flex flex-col items-center">
      <div className="relative w-32 h-32">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className={`text-3xl font-bold ${isLight ? 'text-[#1a1a1a]' : 'text-[#1a1a1a]'}`}>{score}</span>
          <span className={`text-[10px] font-bold uppercase tracking-wider ${isLight ? 'text-[#71717a]' : 'text-[#71717a]'}`}>/100</span>
        </div>
      </div>
      <p className={`mt-2 text-sm font-bold ${isLight ? 'text-slate-700' : 'text-[#71717a]'}`} style={{ color }}>{label}</p>
    </div>
  )
}
  ...
  return (
    <div className="space-y-6">
      ...
      <motion.div
        ...
        className={`rounded-2xl border p-6 ${isLight ? 'bg-white border-gray-200' : 'bg-white border-gray-100 shadow-sm'}`}
      >
        <div className="flex flex-col md:flex-row items-center gap-6">
          <ScoreGauge score={scoreData.compositeScore} theme={theme} />
          <div className="flex-1 space-y-3">
            <h3 className={`text-lg font-bold ${isLight ? 'text-[#1a1a1a]' : 'text-[#1a1a1a]'}`}>Score de satisfaction global</h3>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 pt-2">
              <div className={`text-center px-3 py-2 rounded-lg ${isLight ? 'bg-[#F9F7F1]' : 'bg-gray-50'}`}>
                <p className={`text-xs ${isLight ? 'text-[#71717a]' : 'text-[#71717a]'}`}>Satisfaction</p>
                <p className={`text-sm font-bold ${isLight ? 'text-[#1a1a1a]' : 'text-[#1a1a1a]'}`}>{scoreData.satisfactionRate}%</p>
              </div>
              <div className={`text-center px-3 py-2 rounded-lg ${isLight ? 'bg-[#F9F7F1]' : 'bg-gray-50'}`}>
                <p className={`text-xs ${isLight ? 'text-[#71717a]' : 'text-[#71717a]'}`}>Rapidite</p>
                <p className={`text-sm font-bold ${isLight ? 'text-[#1a1a1a]' : 'text-[#1a1a1a]'}`}>{scoreData.responseTimeScore}%</p>
              </div>
              <div className={`text-center px-3 py-2 rounded-lg ${isLight ? 'bg-[#F9F7F1]' : 'bg-gray-50'}`}>
                <p className={`text-xs ${isLight ? 'text-[#71717a]' : 'text-[#71717a]'}`}>Disponibilite</p>
                <p className={`text-sm font-bold ${isLight ? 'text-[#1a1a1a]' : 'text-[#1a1a1a]'}`}>{scoreData.uptimeScore}%</p>
              </div>
              <div className={`text-center px-3 py-2 rounded-lg ${isLight ? 'bg-[#F9F7F1]' : 'bg-gray-50'}`}>
                <p className={`text-xs ${isLight ? 'text-[#71717a]' : 'text-[#71717a]'}`}>Volume</p>
                <p className={`text-sm font-bold ${isLight ? 'text-[#1a1a1a]' : 'text-[#1a1a1a]'}`}>{scoreData.volumeScore}%</p>
              </div>
            </div>
          </div>
        </div>
...
export const SatisfactionKPI = ({ clientId, theme = 'dark' }) => {
  ...
  return (
    <div className={`group p-6 rounded-2xl border transition-all duration-300 ${
      isLight ? 'bg-white border-gray-200 shadow-sm hover:shadow-md' : 'bg-white border-gray-100 shadow-sm hover:border-gray-300'
    }`}>
      <div className="flex items-center gap-3 mb-6">
        ...
        <h4 className={`text-xs font-bold uppercase tracking-widest ${isLight ? 'text-[#71717a]' : 'text-[#71717a]'}`}>
          Satisfaction IA
        </h4>
      </div>
