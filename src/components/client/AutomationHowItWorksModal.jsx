...
const RoleIcon = ({ role }) => {
  ...
  if (role === 'agent') return <span className="text-[11px] font-bold text-[#0F5F35]">Actero</span>
  if (role === 'system') return <span className="text-[11px] font-bold text-[#9ca3af]">Systeme</span>
  if (role === 'result') return <span className="text-[11px] font-bold text-[#0F5F35]">Resultat</span>
  return null
}
              ...
              <section>
                <div className="flex items-center gap-2 mb-3">
                  <div className="w-7 h-7 rounded-lg bg-[#0F5F35]/10 flex items-center justify-center">
                    <Target className="w-4 h-4 text-[#0F5F35]" />
                  </div>
                ...
                <ul className="space-y-2 pl-9">
                  {content.whatItDoes.map((item, i) => (
                    <li key={i} className="flex gap-2.5 text-[13px] text-[#1a1a1a] leading-relaxed">
                      <span className="text-[#0F5F35] font-bold mt-0.5">-</span>
                      <span>{item}</span>
                    </li>
                  ))}
                </ul>
                    ...
                    return (
                      <div
                        ...
                        className={`flex items-center gap-3 p-3 rounded-xl border ${
                          connected ? 'bg-[#0F5F35]/5 border-[#0F5F35]/20' : optional ? 'bg-[#fafafa] border-[#f0f0f0]' : 'bg-amber-50 border-amber-200'
                        }`}
                      >
                        <div className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 ${
                          connected ? 'bg-[#0F5F35] text-white' : optional ? 'bg-white text-[#9ca3af]' : 'bg-amber-500 text-white'
                        }`}>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            {connected && <span className="text-[10px] font-semibold text-[#0F5F35]">connecte</span>}
                          </div>
                  ...
                  {content.customerStory.map((step, i) => (
                    <div
                      ...
                      className={`p-3.5 rounded-xl ${
                        step.role === 'agent' ? 'bg-[#0F5F35]/5 border border-[#0F5F35]/15' :
                ...
                <ol className="space-y-2.5 pl-9">
                  {content.howToActivate.map((step, i) => (
                    <li key={i} className="flex gap-3">
                      <span className="w-6 h-6 rounded-full bg-[#0F5F35] text-white text-[11px] font-bold flex items-center justify-center flex-shrink-0 mt-0.5">
                        {i + 1}
                      </span>
            ...
            <div className="p-5 md:p-6 border-t border-[#f0f0f0] bg-[#fafafa] flex flex-col sm:flex-row items-center gap-3">
              {onActivate && !isActive && (
                <button
                  ...
                  disabled={!reqsMet}
                  className="w-full sm:w-auto flex items-center justify-center gap-2 px-5 py-2.5 bg-[#0F5F35] hover:bg-[#003725] text-white text-[13px] font-semibold rounded-xl transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
              ...
              {isActive && onViewStats && (
                <button
                  ...
                  className="w-full sm:w-auto flex items-center justify-center gap-2 px-5 py-2.5 bg-[#0F5F35] hover:bg-[#003725] text-white text-[13px] font-semibold rounded-xl transition-colors"
                >
