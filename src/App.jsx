...
const Logo = ({ className = "w-8 h-8", light = false }) => (
  <svg
    ...
    className={`${className} ${light ? "text-white" : "text-white"}`}
  >
...
const Badge = ({ children, variant = "gray", className = "" }) => {
  ...
  return (
    <span
      className={`text-xs font-bold px-2.5 py-1 rounded-lg border ${variants[variant] || variants.gray} ${className}`}
    >
      {children}
    </span>
  );
...
const SkeletonRow = ({ height = "h-4", width = "w-full", className = "" }) => (
  <div
    className={`${height} ${width} bg-white/10 rounded-md animate-pulse ${className}`}
  ></div>
);
          ...
          <div className="flex items-center justify-between mb-8 relative z-10">
            <div>
              ...
              <div className="flex items-baseline gap-2">
                <span
                  className={`text-3xl font-bold tracking-tighter ${isLight ? "text-slate-900" : "text-white"} ${isPositive ? "text-emerald-500" : isNegative ? "text-rose-500" : ""}`}
                >
                  {formattedGrowth}
                </span>
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md flex flex-col items-center">
        ...
        <div className="bg-[#18181b] w-full max-w-[400px] border border-white/5 shadow-2xl rounded-3xl p-8 md:p-10 mx-auto flex flex-col outline outline-1 outline-white/5">
          <div className="flex flex-col items-center mb-8">
            <div
              onClick={() => onNavigate("/")}
              className="cursor-pointer w-12 h-12 rounded-full bg-[#27272a] flex items-center justify-center shadow-inner hover:scale-105 transition-transform mb-4 border border-white/5"
            >
          ...
          <form className="space-y-4 flex flex-col" onSubmit={handleSubmit}>
            {error && (
              <div className="p-3 bg-red-500/10 text-red-500 text-xs font-medium rounded-xl border border-red-500/20 text-center">
                {error}
              </div>
            )}
            {success && (
              <div className="p-3 bg-emerald-500/10 text-emerald-500 text-xs font-medium rounded-xl border border-emerald-500/20 text-center">
                {success}
              </div>
            )}
...
const AdminOnboardingView = () => {
  ...
  const handleCreateAndInvite = async (e) => {
    ...
    try {
      ...
      if (
        ...
        setErrorMsg(
          "Backend required: La RPC 'admin_onboard_client' est manquante ou inaccessible. Demander à Claude de l'ajouter.",
        );
      } else {
        setErrorMsg(
          err.message || "Une erreur est survenue lors de l'onboarding.",
        );
      }
    } finally {
      setLoading(false);
  ...
  const handleResendMagicLink = async (targetEmail) => {
    try {
      const { data, error } = await supabase.rpc("admin_resend_magic_link", {
        p_email: targetEmail,
      });
      if (error) throw error;
      ...
      {successData && (
        <div className="bg-emerald-50 border border-emerald-100 p-8 rounded-2xl shadow-sm animate-fade-in-up">
              ...
              <span className="font-bold text-white">
                {successData.linked ? (
                  ...
                  <span className="text-amber-600 flex items-center gap-1">
                    <Clock className="w-4 h-4" /> En attente de connexion du
                    client
                  </span>
                )}
              </span>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <button
              onClick={() => {
                navigator.clipboard.writeText(successData.client_id);
                alert("ID copié !");
              }}
              className="bg-[#0a0a0a] text-gray-300 hover:bg-white/5 border border-white/10 px-4 py-2.5 rounded-xl text-sm font-bold transition-colors flex items-center justify-center gap-2"
            >
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs text-zinc-500 mt-1">
            Intensité d'exécution des workflows par les clients (30 derniers
            jours)
          </p>
        </div>
          ...
          {filteredClients.length > 0 ? (
            filteredClients.map((client) => (
              ...
              >
                ...
                <kbd className="bg-white/5 border border-white/10 text-zinc-500 text-[10px] px-2 py-0.5 rounded font-mono opacity-0 group-hover:opacity-100 transition-all">
                  Aller au dossier
                </kbd>
              </button>
            ))
    ...
    const load = async () => {
      ...
      try {
        ...
        const { data: requestsRows, error: requestsErr } = await supabase
          .from("requests")
          .select(
            "id, client_id, title, description, stack, priority, status, created_at, clients(brand_name)",
          )
          .order("created_at", { ascending: false });
    ...
    <div className="w-full md:w-64 bg-[#0a0a0a] border-r border-white/10 flex flex-col h-full">
      ...
      <div className="flex-1 py-6 px-4 space-y-1.5 overflow-y-auto">
        ...
        <button
          ...
          className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-bold transition-colors ${activeTab === "clients" ? "bg-white/10 text-white" : "text-gray-400 hover:bg-white/5 hover:text-white"}`}
        >
        ...
        <button
          ...
          className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-bold transition-colors ${activeTab === "automations" ? "bg-white/10 text-white" : "text-gray-400 hover:bg-white/5 hover:text-white"}`}
        >
          <TerminalSquare className="w-4 h-4" /> Infrastructures
        </button>
        <button
          onClick={() => {
            setActiveTab("requests");
        ...
        >
          <div className="flex items-center gap-3">
            <Sparkles className="w-4 h-4" /> Demandes IA
          </div>
        ...
        <button
          ...
          className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-sm font-bold transition-colors ${activeTab === "leads" ? "bg-white/10 text-white" : "text-gray-400 hover:bg-white/5 hover:text-white"}`}
        >
          <div className="flex items-center gap-3">
            <Users className="w-4 h-4" /> Leads AI
          </div>
          {leads.length > 0 && (
        ...
        <button
          ...
          className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-sm font-bold transition-colors ${activeTab === "onboarding" ? "bg-white/10 text-white" : "text-gray-400 hover:bg-white/5 hover:text-white"}`}
        >
          <div className="flex items-center gap-3">
      ...
      <div className="p-4 border-t border-white/10">
        <button
          onClick={onLogout}
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-bold text-gray-400 hover:bg-red-50 hover:text-red-600 transition-colors"
        >
    ...
    <div className="min-h-screen bg-[#030303] flex flex-col md:flex-row font-sans">
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8">
          {activeTab === "onboarding" && <AdminOnboardingView />}
              ...
              <div className="bg-red-50 border border-red-200 rounded-2xl p-5 flex items-start gap-4 shadow-sm">
                ...
                <div>
                  ...
                  <p className="text-sm text-red-700 mt-1 font-medium">
                    Client "DataSync" : 0 exécution depuis 48h. Vérification
                    recommandée.
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-[#0a0a0a] p-6 rounded-2xl border border-white/10 shadow-sm hover:shadow-md transition-shadow">
                  ...
                  <p className="text-4xl font-bold text-white font-mono tracking-tighter">
                    <AnimatedCounter value={4205} />{" "}
                    <span className="text-xl font-medium text-gray-400">h</span>
                  </p>
                </div>
                <div className="bg-[#0a0a0a] p-6 rounded-2xl border border-white/10 shadow-sm hover:shadow-md transition-shadow">
                  <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">
                    Valeur Générée (Globale)
                  </p>
                  <p className="text-4xl font-bold text-white font-mono tracking-tighter">
                    <AnimatedCounter value={185400} />{" "}
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
                <div className="relative w-full sm:w-auto">
                  ...
                  <input
                    ...
                    className="pl-10 pr-4 py-2.5 bg-[#0a0a0a] border border-white/10 rounded-xl text-sm w-full sm:w-80 focus:ring-2 focus:ring-zinc-400 focus:border-zinc-400 outline-none transition-all shadow-sm"
                  />
                </div>
                <button
                  onClick={handleAddClient}
                  className="bg-zinc-300 text-white px-5 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 hover:bg-zinc-400 transition-colors shadow-sm w-full sm:w-auto justify-center"
                >
              ...
              {dataLoading ? (
                ...
                <div className="bg-[#0a0a0a] border border-white/10 rounded-2xl p-16 text-center shadow-sm flex flex-col items-center">
                  ...
                  <p className="text-gray-400 font-normal mb-6">
                    Ajoutez votre premier client pour commencer à monitorer son
                    infrastructure.
                  </p>
                  <button
                    onClick={handleAddClient}
                      ...
                      {clients.map((client) => (
                        ...
                        >
                          ...
                          <td className="px-6 py-4 text-sm">
                            <span className="bg-white/10 text-gray-200 px-3 py-1.5 rounded-lg font-bold border border-white/10">
                              —
                            </span>
                          </td>
                          <td className="px-6 py-4">
                            <span
                              className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold border bg-white/5 text-gray-400 border-white/10 opacity-70 cursor-not-allowed"
                              title="Non disponible avec les données actuelles"
                            >
          ...
          {activeTab === "leads" && (
            <div className="max-w-6xl mx-auto animate-fade-in-up">
              <div className="mb-8">
                ...
                <p className="text-gray-400 font-normal mt-1 flex items-center gap-2">
                  <Sparkles className="w-4 h-4 text-zinc-400" /> Contacts
                  intéressés depuis le simulateur d'Architecture IA sur la
                  landing page.
                </p>
              </div>
              ...
              {dataLoading ? (
                ...
                <div className="bg-[#0a0a0a] border border-white/10 rounded-2xl p-16 text-center shadow-sm flex flex-col items-center">
                  ...
                  <p className="text-gray-400 font-normal mb-6">
                    Patientez jusqu'à ce que de nouveaux prospects soumettent
                    une demande d'Architecture Cible.
                  </p>
                </div>
              ) : (
                <div className="bg-[#0a0a0a] border border-white/10 rounded-2xl shadow-sm overflow-x-auto">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {leads.map((lead) => (
                        ...
                        >
                          ...
                          <td className="px-6 py-4 text-sm">
                            <Badge variant="gray">
                              {lead.source === "landing_architecture"
                                ? "Simulateur IA"
                                : lead.source}
                            </Badge>
                          </td>
          ...
          {activeTab === "automations" && (
            <div className="max-w-6xl mx-auto text-center py-20 animate-fade-in-up">
              ...
              <p className="text-gray-400 font-normal max-w-md mx-auto">
                Cette section vous permettra de connecter l'API n8n pour
                monitorer tous les workflows actifs de vos clients depuis un
                seul endroit.
              </p>
            </div>
          )}
...
const AnimatedCounter = ({
  ...
  return (
    <span ref={elementRef} className={className}>
      {formattedValue}
      {suffix && (
        <span className="text-[0.6em] font-medium text-inherit ml-1 opacity-60 align-baseline">
          {suffix}
        </span>
      )}
    </span>
            ...
            >
              {logs.map((log) => {
                const savedTime = log.time_saved_seconds
                  ...
                  : "";
                const revenue = log.revenue_amount
                  ? `${Number(log.revenue_amount).toLocaleString("fr-FR")}€`
                  : "";
...
const ExecutionPlanDrawer = ({
    ...
    if (startedAt) {
      const fetchRecentEvents = async () => {
        const { data } = await supabase
          .from("automation_events")
          .select(
            "id, event_category, ticket_type, created_at, time_saved_seconds",
          )
  ...
  const steps = {
    growth: [
      {
        title: "Acquisition",
        desc: "Configuration des nouveaux canaux d'acquisition ciblés.",
      },
      {
        title: "Conversion",
        desc: "Déploiement des stratégies d'optimisation du taux de conversion.",
      },
      {
        title: "Analyse",
        desc: "Mise en place des trackers de performance de croissance.",
      },
    ],
    efficiency: [
      {
      ...
      {
        title: "Monitoring",
        desc: "Surveillance continue des gains d'efficacité.",
      },
    ],
    risk: [
      {
        title: "Audit de sécurité",
        desc: "Identification des failles et vulnérabilités potentielles.",
      },
      {
        title: "Mise en place de garde-fous",
        desc: "Déploiement des règles de mitigation des risques.",
      },
      {
        title: "Alerting",
        desc: "Configuration des notifications d'anomalies en temps réel.",
      },
    ],
    automation: [
      {
        title: "Cartographie du workflow",
        desc: "Définition des déclencheurs et actions automatisées.",
      },
      {
        title: "Intégrations requises",
        desc: "Connexion sécurisée aux outils tiers.",
      },
      {
        title: "Assurance Qualité (QA)",
        desc: "Tests approfondis des scénarios d'automatisation.",
      },
    ],
    all: [
      {
        title: "Initialisation",
        desc: "Préparation de l'environnement d'exécution.",
      },
      {
        title: "Déploiement",
        desc: "Mise en oeuvre des recommandations de l'IA.",
      },
                  ...
                  {reco.estimated_revenue_gain > 0 && (
                    <div className="bg-amber-50/50 border border-amber-100 p-4 rounded-xl flex flex-col">
                      ...
                      <span className="text-xl font-bold text-amber-700">
                        +
                        {Number(reco.estimated_revenue_gain).toLocaleString(
                          "fr-FR",
                        )}
                        €
                      </span>
              ...
              {recentEvents.length === 0 ? (
                <div className="py-8 text-center flex flex-col items-center">
                  ...
                  <p className="text-sm font-bold text-gray-400">
                    En attente des premiers signaux n8n...
                  </p>
                </div>
              ) : (
                <div className="space-y-3 mb-6">
                  {recentEvents.map((evt, i) => (
                    <div
                      key={i}
                      className="bg-[#0a0a0a]/5 border border-white/10 p-3 rounded-xl flex items-center justify-between text-sm animate-fade-in-up"
                    >
                      <div className="flex items-center gap-3">
...
const ClientDashboard = ({ onNavigate, onLogout, currentRoute }) => {
  ...
  const Sidebar = () => (
    ...
    >
      ...
      <div className="flex-1 py-6 px-4 space-y-1.5 overflow-y-auto">
        <div className="px-3 text-xs font-bold text-gray-500 uppercase tracking-widest mb-3 mt-2">
          Pilotage
        </div>
        <button
          onClick={() => {
        ...
        <div className="px-3 text-xs font-bold text-gray-500 uppercase tracking-widest mb-3 mt-6">
          Infrastructure
        </div>
        <button
          onClick={() => {
              ...
              {metricsLoading ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  {[...Array(4)].map((_, i) => (
                    <div
                      key={i}
                      className="bg-[#0a0a0a] p-6 rounded-2xl border border-white/10 shadow-[0_8px_30px_rgb(0,0,0,0.04)] animate-pulse flex flex-col justify-between h-40"
                    >
                      <div className="flex items-center gap-3">
                        <SkeletonRow
                          height="h-10"
                ...
                <div className="bg-[#0a0a0a] border border-white/10 rounded-2xl p-16 text-center shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex flex-col items-center">
                  ...
                  <p className="text-gray-400 font-normal">
                    Les premières métriques apparaîtront bientôt dès que vos
                    workflows seront pleinement opérationnels.
                  </p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-zinc-900 rounded-3xl p-8 md:p-10 mt-12 flex flex-col md:flex-row items-center justify-between shadow-xl">
                <div className="mb-6 md:mb-0 text-center md:text-left">
                  ...
                  <p className="text-zinc-500 font-medium">
                    Vous souhaitez ajouter un nouveau processus à votre
                    infrastructure ?
                  </p>
                </div>
                <button
                  onClick={() => setActiveTab("architect")}
                  className="bg-white text-black px-6 py-3.5 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-gray-200 transition-colors shadow-sm w-full md:w-auto"
                >
              ...
              >
                {submitSuccess && (
                  <div className="p-4 bg-emerald-50 text-emerald-600 rounded-xl border border-emerald-100 flex items-center gap-2">
                    <CheckCircle2 className="w-5 h-5 flex-shrink-0" /> Projet
                    soumis avec succès ! Vous serez contacté très prochainement.
                  </div>
                )}
                {submitError && (
                  <div className="p-4 bg-red-50 text-red-600 rounded-xl border border-red-100 flex items-center gap-2">
            ...
            <div className="max-w-4xl mx-auto animate-fade-in-up">
              {!isSupabaseConfigured ? (
                ...
                <div className="bg-[#0a0a0a] border border-white/10 rounded-2xl p-16 text-center shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex flex-col items-center">
                  ...
                  <p className="text-gray-400 font-normal">
                    Les logs de vos automatisations apparaîtront ici
                    prochainement.
                  </p>
                </div>
              ) : (
            ...
            <div className="max-w-4xl mx-auto animate-fade-in-up">
              {!isSupabaseConfigured ? (
                <div className="bg-[#0a0a0a] border border-white/10 rounded-2xl p-16 text-center shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex flex-col items-center">
                  ...
                  <p className="text-gray-400 font-normal">
                    L'IA est en cours d'analyse de vos processus.
                  </p>
                </div>
              ) : (
                <IntelligenceView
          ...
          {activeTab === "reports" && (
            <div className="max-w-4xl mx-auto text-center py-20 animate-fade-in-up">
              ...
              <h3 className="text-2xl font-bold text-white mb-3 tracking-tight">
                Centre de rapports (Bêta)
              </h3>
              <p className="text-zinc-500 font-medium max-w-md mx-auto mb-8 leading-relaxed">
                Générez des rapports PDF mensuels détaillant le ROI exact de
                chaque workflow déployé.
              </p>
              <button
                disabled
                className="opacity-50 cursor-not-allowed bg-zinc-100 text-zinc-500 border border-white/10 shadow-sm px-6 py-3 rounded-xl font-bold inline-flex items-center gap-2"
              >
...
const AIAuditScannerModal = ({ isOpen, onClose, onNavigate }) => {
  ...
  const fetchRealAudit = async (targetUrl) => {
      ...
      bottlenecks: [
        {
          ...
          description:
            "Déploiement d'un agent IA multilingue connecté à votre base de données pour absorber 80% des tickets en temps réel.",
          icon: "bot",
        },
        {
          title: "Abandon de panier inexploité",
          description:
            "Automatisation d'un Voice Agent IA qui rappelle instantanément les paniers premium avec une offre personnalisée.",
          icon: "refresh",
        },
        {
          title: "Saisie manuelle CRM / Facturation",
          description:
            "Synchronisation Make instantanée entre vos paiements (Stripe) et votre comptabilité ou votre CRM de vente.",
          icon: "database",
        },
      ],
    };
    ...
    try {
      // 1. Scrape with Jina
      const jinaUrl = targetUrl.startsWith("http")
        ? targetUrl
        : `https://${targetUrl}`;
      ...
      <div className="relative w-full max-w-4xl max-h-[95vh] overflow-y-auto hide-scrollbar z-10 animate-fade-in-up">
        ...
        <div className="mb-8 text-center mt-6">
          ...
          <p className="text-lg text-gray-400 font-medium max-w-2xl mx-auto">
            Notre Agent IA analyse votre site et extrait les meilleures
            ...
            {scanState === "idle" && (
              ...
              >
                ...
                <p className="text-xs text-zinc-500 mt-4 font-medium flex items-center justify-center gap-1.5">
                  <Lock className="w-3.5 h-3.5" /> Analyse confidentielle et
                  100% gratuite.
                </p>
              </motion.div>
            )}
              ...
              >
                ...
                <p className="text-zinc-400 font-medium mb-10 leading-relaxed">
                  ...
                  <strong className="text-white">
                    3 goulots d'étranglement majeurs
                  </strong>{" "}
                  sur <span className="text-emerald-400 font-mono">{url}</span>.
                  La résolution de ces processus vous ferait économiser{" "}
                  <strong className="text-white bg-white/10 px-2 py-0.5 rounded">
                    {auditData?.timeSaved || "25h+ / semaine"}
                  </strong>
                  .
                </p>
        ...
        responseSchema: {
          ...
          properties: {
            diagnosis: {
              type: "STRING",
              description:
                "Le diagnostic du problème (1 phrase courte et percutante)",
            },
            solution: {
              type: "STRING",
              description:
                "La logique de la solution d'automatisation proposée (ex: Déclencheur X -> Action Y avec l'outil Z)",
            },
            timeSaved: {
              type: "STRING",
              description:
                "Estimation réaliste du temps gagné (ex: '15h / mois')",
            },
            revenueImpact: {
              type: "STRING",
              description:
                "Impact métier (ex: '+12% de conversion sur les paniers abandonnés')",
            },
          },
        ...
        <main>
          ...
          <div className="relative w-full z-10">
            <div className="relative z-10 w-full">
                      ...
                      <div className="relative bg-[#0a0a0a] rounded-[28px] p-8 md:p-10 border border-white/5 h-full overflow-hidden hover:border-white/15 transition-all duration-500">
                        ...
                        <div className="relative z-10">
                          ...
                          <p className="text-gray-400 font-medium leading-relaxed max-w-md text-base">
                            Actero se connecte à l'ensemble de votre stack
                            (Shopify, CRM, Support) et surveille chaque
                            interaction en temps réel. Aucune installation
                            technique nécessaire.
                          </p>
                          <div className="mt-8 flex gap-3 flex-wrap">
                            {["Shopify", "Klaviyo", "Zendesk", "Hubspot"].map(
                              (tag) => (
                                <span
                                  key={tag}
                                  className="text-xs font-semibold bg-white/5 text-gray-400 border border-white/10 px-3 py-1.5 rounded-full"
                                >
                                  {tag}
                                </span>
                              ),
                      ...
                      <div className="relative bg-[#0a0a0a] rounded-[28px] p-8 border border-white/5 h-full overflow-hidden hover:border-white/15 transition-all duration-500">
                        ...
                        <div className="relative z-10 flex flex-col h-full">
                          ...
                          <p className="text-gray-400 font-medium leading-relaxed text-base flex-1">
                            ...
                            exact qui augmentera vos marges et rédige le plan
                            d'action.
                          </p>
                          <div className="mt-6 w-full h-px bg-gradient-to-r from-transparent via-white/10 to-transparent"></div>
                          <p className="mt-4 text-sm font-bold text-zinc-500 italic">
                            "Gemini a analysé 1,247 flux pour cette
                            recommandation."
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="relative bg-[#0a0a0a] rounded-[28px] p-8 md:p-10 border border-white/5 h-full overflow-hidden hover:border-white/15 transition-all duration-500">
                        ...
                        <div className="relative z-10">
                          ...
                          <p className="text-gray-400 font-medium leading-relaxed max-w-md text-base">
                            ...
                            déploie sans aucun code ni intervention de votre
                            part.
                          </p>
                        </div>
                      </div>
                        ...
                        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center gap-6">
                          <div className="flex-1">
                            ...
                            <p className="text-xl md:text-2xl font-bold text-white leading-snug">
                              Un système autonome qui optimise vos marges{" "}
                              <span className="text-zinc-400">24h/24</span>,
              ...
              <section
                id="proof"
                className="py-24 bg-transparent px-6 relative z-10"
              >
                  ...
                  <FadeInUp className="text-center mb-16">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-0 max-w-4xl mx-auto">
                      {[
                        ...
                        <div
                          key={i}
                          className={`flex flex-col items-center justify-center py-6 ${i < 2 ? "md:border-r border-white/5" : ""}`}
                        >
                          <ScrollCounter
                            value={stat.value}
                            prefix={stat.prefix}
                    ...
                    <p className="text-sm font-semibold text-gray-400 mt-12 uppercase tracking-wide">
                      Résultats moyens observés chez nos clients E-commerce.
                    </p>
                  </FadeInUp>
                    ...
                    <div className="bg-[#030303] rounded-[32px] p-8 md:p-12 border border-white/10 shadow-xl relative overflow-hidden group">
                      ...
                      <div className="relative z-10">
                        <div className="flex items-center gap-3 mb-10">
                          <div className="px-3 py-1 bg-[#0a0a0a] border border-white/5 rounded-full text-xs font-bold uppercase tracking-widest text-zinc-300 shadow-sm">
                            Étude de Cas
                          </div>
                        ...
                        <div className="grid md:grid-cols-2 gap-12 items-center">
                          <div className="space-y-8">
                            <div>
                              ...
                              <p className="text-xl font-bold text-white leading-tight">
                                Moteur de recommandation post-achat avec SMS
                                dynamiques (Klaviyo + Shopify).
                              </p>
                            </div>
                          ...
                          <div className="bg-[#0a0a0a] rounded-3xl p-8 border border-white/5 shadow-[0_10px_40px_rgba(0,0,0,0.04)] text-center">
                            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">
                              Revenus nets générés en 30 jours
                            </p>
                            <p className="text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-br from-emerald-500 to-emerald-700 tracking-tighter mb-6">
                              + 12 400 €
                            </p>
                            <button
                  ...
                  <SlideInLeft className="bg-[#0a0a0a] rounded-[32px] border border-white/10 overflow-hidden shadow-sm">
                    <div className="grid grid-cols-2 md:grid-cols-2 border-b border-white/5 bg-white/5/50">
                      <div className="p-6 md:p-8 border-r border-white/5">
                        <p className="text-lg font-bold tracking-tight text-gray-400 line-through decoration-gray-300">
                          Make / Zapier
                        </p>
                      </div>
                    ...
                    <div className="divide-y divide-gray-100">
                      {[
                        {
                          old: "Création manuelle des flux",
                          new: "Déploiement Autonome piloté par l'IA",
                        },
                        {
                          old: "Focalisé sur la technique (API, Webhooks)",
                          new: "Focalisé sur le Business (Marges, AOV, LTV)",
                        },
                        {
                          old: "Maintenance constante en cas de bug",
                          new: "Monitoring intelligent et auto-réparation",
                        },
                        {
                          old: "Aucun suivi du ROI chiffré",
                          new: "Impact financier mesuré sur chaque workflow",
                        },
                      ].map((row, i) => (
                        <div
                          key={i}
                          className="grid grid-cols-2 group hover:bg-white/5/30 transition-colors"
                        >
              ...
              >
                <div className="max-w-3xl mx-auto">
                  ...
                  <div className="space-y-4">
                    {[
                      {
                        q: "Est-ce que je dois savoir coder pour utiliser Actero ?",
                        a: "Absolument pas. Actero est une plateforme 'Done-for-you' en arrière-plan. Vous validez simplement les recommandations de notre IA depuis votre interface, et nos ingénieurs (ou nos scripts) s'occupent du déploiement technique.",
                      },
                      {
                        q: "Combien de temps prend l'intégration avec ma boutique ?",
                        a: "Moins de 5 minutes. Une fois votre compte créé, il suffit d'accorder les permissions Shopify/Klaviyo. Le premier audit de vos flux prend environ 24 heures.",
                      },
                      {
                        q: "Et si ça casse mon site existant ?",
                        a: "Impossible. Nous opérons en aval via API. Nos automatisations ne modifient pas le code frontal de votre site (Liquid/React), mais optimisent le traitement des données dans vos outils marketing et logistiques.",
                      },
                      {
                        q: "Puis-je annuler à tout moment ?",
                        a: "Oui, nos plans Starter et Growth sont sans engagement. Vous pouvez annuler d'un simple clic depuis vos paramètres.",
                      },
                    ].map((faq, i) => (
                      <div
                        key={i}
                        className="border border-white/10 rounded-2xl bg-[#030303] overflow-hidden transition-all duration-300"
                      >
                        <button
                          onClick={() =>
                        ...
                        >
                          ...
                          <div
                            className={`w-8 h-8 rounded-full border flex items-center justify-center flex-shrink-0 transition-transform duration-300 ${openFaqIndex === i ? "bg-zinc-300 border-zinc-300 text-white rotate-180" : "bg-[#0a0a0a] border-white/10 text-gray-400"}`}
                          >
                            <svg
                              className="w-4 h-4"
                              fill="none"
                        ...
                        <div
                          className={`px-6 overflow-hidden transition-all duration-500 ease-in-out ${openFaqIndex === i ? "max-h-96 pb-6 opacity-100" : "max-h-0 opacity-0"}`}
                        >
          ...
          {/* Close relative w-full z-10 dotted surface wrapper */}
        </main>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative bg-[#0a0a0a] rounded-2xl p-8 max-w-md w-full shadow-2xl animate-fade-in-up z-10 border border-white/5">
              <button
                ...
                className="absolute top-6 right-6 text-gray-400 hover:text-gray-400 transition-colors disabled:opacity-50"
              >
                <X className="w-5 h-5" />
              </button>
              ...
              <p className="text-gray-400 font-medium text-sm mb-6">
                Laissez-nous vos coordonnées pour générer votre architecture
                cible personnalisée.
              </p>
              ...
              <form onSubmit={handleModalSubmit} className="space-y-5">
                ...
                <div className="pt-2 flex items-center justify-end gap-3">
                  <button
                    ...
                    onClick={closeModal}
                    disabled={isSubmitting}
                    className="px-5 py-3 rounded-xl font-semibold text-gray-400 hover:text-white hover:bg-white/5 transition-colors disabled:opacity-50"
                  >
                    Annuler
                  </button>
                  <button
...
const DashboardGate = ({ onNavigate, onLogout, currentRoute }) => {
  ...
  const dLog = (...args) => {
    if (isDebug) console.log("[DASHBOARD_GATE]", ...args);
  };
  ...
  useEffect(() => {
    ...
    const checkTenant = async () => {
      try {
        ...
        dLog("Session status: Authenticated", {
  ...
  // Loading: show spinner while we determine the role
        ...
        <nav className="w-full bg-[#0d0d0d]/70 backdrop-blur-3xl border border-white/10 shadow-2xl rounded-full transition-all duration-300">
          <div className="px-6 md:px-8 h-14 md:h-[60px] flex justify-between items-center">
            ...
            <div className="flex items-center gap-3">
              <button
                onClick={() => onNavigate("/login")}
                className="hidden lg:block text-sm font-semibold text-gray-400 hover:text-white transition-colors"
              >
                Connexion
              </button>
              <div className="hidden sm:block scale-90 origin-right">
            ...
            >
              {[
                ...
                <button
                  key={i}
                  onClick={item.action}
                  className={`w-full text-left px-4 py-3 rounded-xl text-base font-semibold ${item.label === "Entreprise" ? "text-white bg-white/5" : "text-gray-300 hover:text-white hover:bg-white/5"} transition-all`}
                >
                  {item.label}
                </button>
              ))}
      ...
      <main className="pt-32 pb-20 px-6">
        ...
        <div className="max-w-5xl mx-auto text-center mb-32">
          ...
          <p className="text-xl md:text-2xl text-gray-400 font-medium max-w-3xl mx-auto leading-relaxed">
            Les équipes e-commerce n'ont pas besoin de plus d'outils ni de plus
            de réunions. Elles ont besoin d'un{" "}
            <strong className="text-white">système</strong>. Actero connecte
            l'exécution, l'itération et la performance dans un seul workflow.
          </p>
        </div>
          ...
          <div className="grid md:grid-cols-2 gap-16 items-center">
            <div>
              <h2 className="text-3xl md:text-4xl font-bold tracking-tight mb-6">
                Votre marketing devrait tourner comme un logiciel.
              </h2>
              <p className="text-lg text-gray-400 leading-relaxed font-medium mb-6">
                Nous avons créé Actero parce que les vieilles solutions sont
                cassées : les agences scalent lentement, le recrutement est
                lourd, et les outils déconnectés créent des angles morts.
              </p>
              <p className="text-lg text-gray-400 leading-relaxed font-medium">
                Notre mission est de remplacer cette complexité par une
                infrastructure unifiée, pilotée par l'IA, qui fait croître votre
                business pendant que vous dormez.
              </p>
            </div>
              ...
              <div className="space-y-8 relative z-10">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-400">
                      Votre infrastructure est opérationnelle en une semaine,
                      pas en 3 mois.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-400">
                      Des agents autonomes qui gèrent support, relances et
                      segments 24/7.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-400">
                      Chaque automatisation est mesurée et optimisée par les
                      données, pas par l'intuition.
                    </p>
                  </div>
                </div>
              </div>
        ...
        <div className="max-w-6xl mx-auto mb-32">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {values.map((v, i) => (
              <div
                key={i}
                className="bg-[#0a0a0a] border border-white/[0.08] rounded-2xl p-8 hover:border-white/20 transition-all group"
              >
        ...
        <div className="max-w-4xl mx-auto text-center bg-gradient-to-b from-[#0a0a0a] to-[#030303] border border-white/10 rounded-3xl p-12 md:p-16 relative overflow-hidden">
          ...
          <h2 className="text-3xl md:text-4xl font-bold tracking-tight mb-4 relative z-10">
            Rejoignez l'infrastructure du futur.
          </h2>
          <p className="text-gray-400 font-medium mb-8 max-w-lg mx-auto relative z-10">
            Discutons de comment Actero peut transformer vos opérations
            e-commerce.
          </p>
          <button
            onClick={() => onNavigate("/")}
            className="bg-white text-black px-8 py-4 rounded-xl font-bold hover:scale-105 transition-transform shadow-[0_0_30px_rgba(255,255,255,0.1)] relative z-10"
          >
...
const PricingPage = ({ onNavigate }) => {
  ...
  const faqs = [
    ...
    },
    {
      ...
      a: "Absolument. Nous utilisons un chiffrement AES-256, des audits SOC 2 réguliers, et vos données ne sont jamais partagées avec des tiers. Vous gardez le contrôle total.",
    },
    {
    ...
    },
    {
      ...
      a: "Nous nous connectons nativement à Shopify, Klaviyo, Gorgias, Make, Zapier, et des dizaines d'autres outils. Des intégrations custom sont disponibles sur le plan Scale.",
    },
  ];
    ...
    <div className="min-h-screen bg-[#030303] text-white font-sans selection:bg-white/20">
            ...
            >
              {[
                ...
                <button
                  key={i}
                  onClick={item.action}
                  className={`w-full text-left px-4 py-3 rounded-xl text-base font-semibold ${item.label === "Tarification" ? "text-white bg-white/5" : "text-gray-300 hover:text-white hover:bg-white/5"} transition-all`}
                >
                  {item.label}
                </button>
              ))}
      ...
      <main className="pt-32 pb-20 px-6">
        ...
        <div className="max-w-5xl mx-auto text-center mb-20">
          ...
          <h1 className="text-5xl md:text-7xl font-extrabold tracking-tighter mb-8 bg-gradient-to-b from-white to-gray-500 bg-clip-text text-transparent">
            Des formules calibrées
            <br />
            pour votre croissance.
          </h1>
          <p className="text-xl md:text-2xl text-gray-400 font-medium max-w-3xl mx-auto leading-relaxed">
            Choisissez un plan adapté à votre chiffre d'affaires. Un système
            unique pour une croissance rentable sur Shopify.
          </p>
        </div>
        ...
        <div className="max-w-6xl mx-auto grid md:grid-cols-3 gap-6 mb-32">
          {plans.map((plan, i) => (
            ...
            >
              ...
              <p className="text-gray-400 font-medium leading-relaxed mb-8 flex-shrink-0">
                {plan.description}
              </p>
...
const FaqPage = ({ onNavigate }) => {
  ...
  const faqs = [
    ...
    {
      q: "Combien de temps prend le déploiement ?",
      a: "En moyenne 7 jours ouvrés pour notre infrastructure standard. Le forfait sur mesure nécessite un onboarding plus approfondi de 2-3 semaines.",
    },
    {
      q: "Quelles intégrations supportez-vous ?",
      a: "Nous nous connectons nativement à Shopify, Klaviyo, Gorgias, Make, Zapier, et des dizaines d'autres outils. Des intégrations custom sont disponibles sur demande.",
    },
    {
      q: "L'agent IA peut-il gérer des réclamations complexes ?",
      a: "Notre agent de Niveau 1 résout automatiquement ~70% des requêtes (suivi de colis, retours d'infos simples). Les cas complexes ou à forte charge émotionnelle sont escaladés instantanément à un agent humain avec tout le contexte de la conversation.",
    },
    {
      q: "L'IA peut-elle s'exprimer avec le ton de ma marque ?",
      a: "Oui, lors du déploiement, nous injectons vos guidelines de marque, vos historiques de tickets et votre ligne éditoriale pour que l'agent imite parfaitement votre voix vis-à-vis de vos clients.",
    },
    {
      q: "Qu'est-ce qui vous différencie d'une simple agence ?",
      a: "Nous ne sommes pas des consultants, nous construisons des infrastructures. Nous installons un système propriétaire connecté à votre e-commerce pour que votre croissance soit mécanisée et non plus dépendante uniquement d'efforts humains de dernière minute.",
    },
  ];
    ...
    <div className="min-h-screen bg-[#030303] text-white font-sans selection:bg-white/20">
            ...
            >
              {[
                ...
                <button
                  key={i}
                  onClick={item.action}
                  className={`w-full text-left px-4 py-3 rounded-xl text-base font-semibold ${item.label === "FAQ" ? "text-white bg-white/5" : "text-gray-300 hover:text-white hover:bg-white/5"} transition-all`}
                >
                  {item.label}
                </button>
              ))}
      ...
      <main className="pt-32 pb-20 px-6">
        ...
        <div className="max-w-5xl mx-auto text-center mb-20">
          ...
          <p className="text-xl md:text-2xl text-gray-400 font-medium max-w-3xl mx-auto leading-relaxed">
            Trouvez rapidement toutes les réponses concernant notre architecture
            complète d'automatisation d'entreprise.
          </p>
        </div>
          ...
          <div className="space-y-3">
            {faqs.map((faq, i) => (
              ...
              >
                ...
                >
                  <span className="font-bold text-lg md:text-xl text-white pr-4">
                    {faq.q}
                  </span>
                  <div
                    className={`flex-shrink-0 transition-transform duration-300 bg-white/5 border border-white/10 rounded-full p-2 group-hover:bg-white/10 ${openFaq === i ? "rotate-45" : ""}`}
                  >
                    <Plus className="w-5 h-5 text-gray-400" />
                  </div>
                </button>
        ...
        <div className="max-w-4xl mx-auto text-center bg-gradient-to-b from-[#0a0a0a] to-[#030303] border border-white/10 rounded-3xl p-12 md:p-16 relative overflow-hidden">
          ...
          <p className="text-gray-400 font-medium mb-8 max-w-lg mx-auto relative z-10">
            Réservez un appel gratuit avec un ingénieur Actero pour auditer
            ensemble vos opérations.
          </p>
          <a
            href="mailto:contact@actero.fr?subject=Demande de rendez-vous - Actero"
            className="inline-block bg-white text-black px-8 py-4 rounded-xl font-bold hover:scale-105 transition-transform shadow-[0_0_30px_rgba(255,255,255,0.1)] relative z-10"
          >
            Nous contacter
          </a>
        </div>
      ...
      <div className="min-h-screen flex items-center justify-center bg-white/5 font-sans">
        <div className="text-center p-12 bg-[#0a0a0a] border border-white/10 rounded-3xl shadow-2xl max-w-md w-full mx-6">
          ...
          <p className="text-gray-400 font-medium mb-8 leading-relaxed">
            Merci pour votre confiance. Pour configurer votre infrastructure,
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-white/5 font-sans">
        <div className="text-center p-12 bg-[#0a0a0a] border border-white/10 rounded-3xl shadow-2xl max-w-md w-full mx-6">
          ...
          <p className="text-gray-400 font-medium mb-8 leading-relaxed">
            Vous avez annulé le processus de paiement. Aucun prélèvement n'a été
            effectué. Vous pouvez réessayer à tout moment.
          </p>
          <div className="flex gap-4">
            <button
              onClick={() => navigate("/tarifs")}
              className="flex-1 bg-white/5 border border-white/10 text-white px-6 py-4 rounded-xl font-bold hover:bg-white/10 transition-colors"
            >
              Retour aux tarifs
            </button>
            <button
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-white/5 font-sans">
      <div className="text-center p-8 bg-[#0a0a0a] border border-white/10 rounded-3xl shadow-sm">
        ...
        <button
          onClick={() => navigate("/")}
          className="bg-white text-zinc-900 px-6 py-3 rounded-xl font-bold hover:bg-zinc-800 transition-colors mt-6"
        >
          Retour à l'accueil
        </button>
      </div>
