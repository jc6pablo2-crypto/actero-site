...
const StatusBadge = ({ status, config }) => {
  const s = config[status] || { label: status, color: 'bg-gray-500/20 text-[#716D5C]' }
  return (
...
const KPICard = ({ label, value, icon: Icon, accent = 'emerald', subtitle }) => (
  <div className="p-5 rounded-2xl bg-[#F9F7F1] border border-gray-200 hover:border-gray-300 transition-all group">
    ...
    {subtitle && <p className="text-xs text-[#716D5C] mt-1">{subtitle}</p>}
  </div>
)
...
const CopyButton = ({ text }) => {
  ...
  return (
    <button
      onClick={handleCopy}
      className="flex items-center gap-2 px-4 py-2 rounded-xl bg-gray-50 border border-gray-200 text-sm font-bold text-[#716D5C] hover:bg-gray-50 transition-all"
    >
...
const J30Badge = ({ eligibilityDate }) => {
  ...
  if (daysLeft === null) return <span className="text-xs text-[#716D5C]">—</span>
  if (isEligible) {
  ...
  if (!events || events.length === 0) {
    return (
      <div className="text-center py-6">
        ...
        <p className="text-sm text-[#716D5C]">Aucun événement pour ce lead.</p>
      </div>
    )
  }
  return (
    <div className="relative pl-6 space-y-4">
      <div className="absolute left-2.5 top-2 bottom-2 w-px bg-gray-50" />
      {events.map((ev, i) => (
        <div key={ev.id || i} className="relative flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            {ev.note && <p className="text-xs text-[#716D5C] mt-0.5">{ev.note}</p>}
            <p className="text-[10px] text-[#716D5C] mt-1">
...
export const AmbassadorDashboard = ({ onNavigate, currentRoute }) => {
    ...
    switch (activeTab) {
        ...
        return (
          <div className="space-y-8">
            <div>
              ...
              <p className="text-[#716D5C] font-medium">Votre tableau de bord ambassadeur.</p>
            </div>
            ...
            <div className="p-5 rounded-2xl bg-gradient-to-r from-emerald-500/5 to-cyan-500/5 border border-emerald-500/10">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h3 className="text-sm font-bold text-[#262626] mb-1">Comment ça marche</h3>
                  <p className="text-xs text-[#716D5C] leading-relaxed">
                    <strong className="text-[#716D5C]">1.</strong> Partagez votre lien unique &rarr;{' '}
                    <strong className="text-[#716D5C]">2.</strong> On s’occupe de la vente &rarr;{' '}
                    <strong className="text-[#716D5C]">3.</strong> Vous êtes récompensé 30j après le paiement client.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="p-6 rounded-2xl bg-[#F9F7F1] border border-gray-200">
              ...
              <p className="text-xs text-[#716D5C] mt-3">Code : {ambCode}</p>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <div className={`p-5 rounded-2xl border text-center relative overflow-hidden ${leadsWon >= 3 ? 'bg-amber-500/5 border-amber-500/20' : 'bg-gray-50 border-gray-100'}`}>
                ...
                <p className="text-xs text-[#716D5C] mt-1">par client signé</p>
                {leadsWon < 3 && <p className="text-[10px] text-[#716D5C] mt-2">Encore {3 - leadsWon} client{3 - leadsWon > 1 ? 's' : ''} pour débloquer</p>}
              </div>
            </div>
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-bold text-[#262626]">Recommander un prospect</p>
                      <p className="text-xs text-[#716D5C]">Soumettez un nouveau lead</p>
                    </div>
                  </div>
                  <ChevronRight className="w-5 h-5 text-[#716D5C] group-hover:text-emerald-400 transition-colors" />
                </div>
              </button>
              <button
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-bold text-[#262626]">Mes commissions</p>
                      <p className="text-xs text-[#716D5C]">Suivez vos récompenses</p>
                    </div>
                  </div>
                  <ChevronRight className="w-5 h-5 text-[#716D5C] group-hover:text-[#262626] transition-colors" />
                </div>
              </button>
            </div>
          </div>
      ...
      case 'link':
        return (
          <div className="space-y-8">
            <div>
              <h1 className="text-3xl font-bold text-[#262626] mb-2">Mon Lien</h1>
            ...
            <div className="p-8 rounded-2xl bg-[#F9F7F1] border border-gray-200 space-y-6">
              ...
              <div>
                <p className="text-xs font-bold text-[#716D5C] uppercase tracking-wider mb-3">Lien de parrainage</p>
                <div className="flex items-center gap-3 p-4 rounded-xl bg-gray-50 border border-gray-200 flex-wrap">
                  <code className="text-sm text-[#262626] font-mono break-all flex-1">{ambassadorLink || '—'}</code>
                  {ambassadorLink && <CopyButton text={ambassadorLink} />}
                  {ambassadorLink && (
                    <button
                      onClick={handleShare}
              ...
              <div className="p-5 rounded-xl bg-emerald-500/5 border border-emerald-500/10">
                <p className="text-sm text-[#716D5C] font-medium leading-relaxed">
              ...
              <div className="p-5 rounded-xl bg-gray-50 border border-gray-200">
                <p className="text-xs font-bold text-[#716D5C] uppercase tracking-wider mb-2">Conditions de commission</p>
                <p className="text-sm text-[#716D5C] font-medium leading-relaxed">
        ...
        return (
          <div className="space-y-8">
            <div className="flex items-center justify-between flex-wrap gap-4">
              <div>
                <h1 className="text-3xl font-bold text-[#262626] mb-2">Mes Recommandations</h1>
                <p className="text-[#716D5C] font-medium">{leads.length} recommandation{leads.length > 1 ? 's' : ''} au total</p>
              </div>
              <button
            ...
            {leads.length === 0 ? (
              <div className="rounded-2xl bg-[#F9F7F1] border border-gray-200 p-12 text-center">
                ...
                <h3 className="text-lg font-bold text-[#262626] mb-2">Aucune recommandation</h3>
                <p className="text-[#716D5C] font-medium max-w-sm mx-auto mb-6">
                ...
                {leads.map((lead) => (
                  <div key={lead.id} className="rounded-2xl bg-[#F9F7F1] border border-gray-200 hover:border-gray-300 transition-all p-5">
                    <div className="flex items-start justify-between gap-4 mb-3">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 flex-wrap">
                          <p className="text-sm font-bold text-[#262626]">{lead.prospect_name}</p>
                          <StatusBadge status={lead.status} config={LEAD_STATUS_MAP} />
                        </div>
                        <p className="text-xs text-[#716D5C] mt-1">
                          {lead.company_name}
                          {lead.company_niche && <span className="ml-2 px-2 py-0.5 rounded-full bg-gray-50 text-[#716D5C] text-[10px] font-bold">{lead.company_niche}</span>}
                        </p>
                      </div>
                      <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        <button
                          ...
                          className="p-1.5 rounded-lg hover:bg-gray-50 transition-colors text-[#716D5C] hover:text-[#262626]"
                    ...
                    {lead.status_note && (
                      <p className="text-xs text-[#716D5C] mt-2 italic">{lead.status_note}</p>
                    )}
                      ...
                      {selectedLeadForTimeline === lead.id && (
                        ...
                        >
                          <div className="mt-4 pt-4 border-t border-gray-100">
                            <p className="text-xs font-bold text-[#716D5C] uppercase tracking-wider mb-3">Timeline</p>
                            {timelineLoading ? (
                  ...
                  >
                    {leadSuccess ? (
                      <div className="text-center py-8">
                        ...
                        <h3 className="text-xl font-bold text-[#262626] mb-2">Recommandation envoyée !</h3>
                        <p className="text-sm text-[#716D5C]">Notre équipe va traiter votre lead.</p>
                      </div>
                    ) : (
                      <>
                        <div className="flex items-center justify-between mb-6">
                          <h3 className="text-xl font-bold text-[#262626]">Recommander un prospect</h3>
                          <button onClick={() => setShowLeadModal(false)} className="text-[#716D5C] hover:text-[#262626]">
                            <X className="w-5 h-5" />
                          </button>
                        </div>
                        ...
                        <form onSubmit={handleLeadSubmit} className="space-y-4">
                          <div>
                            <label className="block text-sm font-bold text-[#716D5C] mb-1.5">Nom du prospect *</label>
                            <input
                              type="text" required
                          ...
                          <div>
                            <label className="block text-sm font-bold text-[#716D5C] mb-1.5">Entreprise *</label>
                            <input
                              type="text" required
                          ...
                          <div>
                            <label className="block text-sm font-bold text-[#716D5C] mb-1.5">Niche</label>
                            <select
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              <label className="block text-sm font-bold text-[#716D5C] mb-1.5">Email</label>
                              <input
                                type="email"
                                value={leadForm.prospect_email}
                                onChange={(e) => setLeadForm((p) => ({ ...p, prospect_email: e.target.value }))}
                                className="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 text-[#262626] placeholder-gray-500 focus:outline-none focus:border-emerald-500/50 transition-all"
                                placeholder="email@exemple.com"
                              />
                            </div>
                            <div>
                              <label className="block text-sm font-bold text-[#716D5C] mb-1.5">Téléphone</label>
                              <input
                                type="tel"
                          ...
                          <div>
                            <label className="block text-sm font-bold text-[#716D5C] mb-1.5">Message</label>
                            <textarea
                              value={leadForm.message}
                              onChange={(e) => setLeadForm((p) => ({ ...p, message: e.target.value }))}
                              rows={3}
                              className="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 text-[#262626] placeholder-gray-500 focus:outline-none focus:border-emerald-500/50 transition-all resize-none"
      ...
      case 'commissions':
        return (
          <div className="space-y-8">
            <div>
              <h1 className="text-3xl font-bold text-[#262626] mb-2">Mes Commissions</h1>
              <p className="text-[#716D5C] font-medium">Suivi de vos récompenses.</p>
            </div>
            ...
            {commissions.length === 0 ? (
              <div className="rounded-2xl bg-[#F9F7F1] border border-gray-200 p-12 text-center">
                ...
                <h3 className="text-lg font-bold text-[#262626] mb-2">Aucune commission</h3>
                <p className="text-[#716D5C] font-medium max-w-sm mx-auto">
                ...
                {commissions.map((c) => (
                  <div key={c.id} className="rounded-2xl bg-[#F9F7F1] border border-gray-200 hover:border-gray-300 transition-all p-5">
                    <div className="flex items-center justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 flex-wrap">
                          <p className="text-sm font-bold text-[#262626]">{c.prospect_name || c.ambassador_leads?.prospect_name || '—'}</p>
                          <StatusBadge status={c.status} config={COMMISSION_STATUS_MAP} />
                        </div>
                        <p className="text-xs text-[#716D5C] mt-1">{c.company_name || c.ambassador_leads?.company_name || ''}</p>
                      </div>
                      <div className="flex items-center gap-4 flex-shrink-0">
                        <J30Badge eligibilityDate={c.eligibility_date} />
                        <p className="text-lg font-bold font-mono text-[#262626]">
                          {c.amount ? `${Number(c.amount).toLocaleString('fr-FR')}  €` : '—'}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-4 mt-3 text-xs text-[#716D5C]">
      ...
      case 'rules':
        return (
          <div className="space-y-8">
            <div>
              <h1 className="text-3xl font-bold text-[#262626] mb-2">Règles du programme</h1>
              <p className="text-[#716D5C] font-medium">Tout ce que vous devez savoir.</p>
            </div>
                ...
                <div className="space-y-3">
                  {[
                    ...
                    <div key={i} className="flex items-start gap-3">
                      ...
                      <p className="text-sm text-[#716D5C] leading-relaxed">{rule}</p>
                    </div>
                  ))}
                </div>
              </div>
      ...
      case 'profile':
        return (
          <div className="space-y-8">
            <div>
              <h1 className="text-3xl font-bold text-[#262626] mb-2">Mon Profil</h1>
              <p className="text-[#716D5C] font-medium">Gérez vos informations personnelles.</p>
            </div>
            ...
            {profileForm && (
              <form onSubmit={handleProfileSave} className="max-w-xl space-y-6">
                <div className="grid sm:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-sm font-bold text-[#716D5C] mb-2">Prénom</label>
                    <input
                      type="text"
                      value={profileForm.first_name}
                      onChange={(e) => setProfileForm((p) => ({ ...p, first_name: e.target.value }))}
                      className="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 text-[#262626] focus:outline-none focus:border-emerald-500/50 transition-all"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-[#716D5C] mb-2">Nom</label>
                    <input
                      type="text"
                ...
                <div>
                  ...
                  <input
                    ...
                    disabled
                    className="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 text-[#716D5C] cursor-not-allowed"
                  />
                  <p className="text-xs text-[#716D5C] mt-1">L'email ne peut pas être modifié.</p>
                </div>

                <div>
                  <label className="block text-sm font-bold text-[#716D5C] mb-2">Téléphone</label>
                  <input
                    type="tel"
                ...
                <div>
                  <label className="block text-sm font-bold text-[#716D5C] mb-2">SIRET (optionnel)</label>
                  <input
                    type="text"
                ...
                <div className="p-5 rounded-xl bg-gray-50 border border-gray-100">
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="block text-sm font-bold text-[#716D5C] mb-2">Titulaire du compte</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="block text-sm font-bold text-[#716D5C] mb-2">IBAN</label>
                      <input
                        type="text"
                        value={profileForm.iban}
                        onChange={(e) => setProfileForm((p) => ({ ...p, iban: e.target.value.toUpperCase() }))}
                        className="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 text-[#262626] placeholder-gray-500 focus:outline-none focus:border-emerald-500/50 transition-all font-mono"
                        placeholder="FR76 1234 5678 9012 3456 7890 123"
                      />
                    </div>
                    <div>
                      <label className="block text-sm font-bold text-[#716D5C] mb-2">BIC / SWIFT (optionnel)</label>
                      <input
                        type="text"
                ...
                <div className="grid sm:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-sm font-bold text-[#716D5C] mb-2">Date d'inscription</label>
                    <p className="px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 text-[#716D5C] text-sm">
                      {ambassador?.created_at ? new Date(ambassador.created_at).toLocaleDateString('fr-FR') : '—'}
                    </p>
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-[#716D5C] mb-2">Statut du compte</label>
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-[#262626] flex">
      {/* Mobile menu toggle */}
      <button
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-40 bg-gray-500 md:hidden"
